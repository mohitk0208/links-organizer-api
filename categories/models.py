...
class Category(models.Model):
    ...
    def get_access_level(self, user_id: int):
        try:
            access = self.access.through.objects.get(category_id=self.id, user_id=user_id)
...
class CategoryAccess(models.Model):
    ...
    level = models.IntegerField(choices=AccessLevel.choices, default=AccessLevel.READ_ONLY)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['category', 'user'], name='unique_category_access_through')
        ]
